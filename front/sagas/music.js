...
//요청리스트 가져오기
function boardListAPI(data) {
	console.log("data?", data);
	return axios.get(
		`/music/getBoardList?lastId=${data.lastId || 0}&insertEnd=${
			data.insertEnd || "N"
		}`,
	);
}
...
function* boardList(action) {
	try {
		const result = yield call(boardListAPI, action);
...
//요청 상세 정보
function boardInfoAPI(id) {
	return axios.get(`/music/boardInfo?id=${id || ""}`);
}

function* boardInfo(action) {
	try {
		const result = yield call(boardInfoAPI, action.id);
		console.log("boardInfoAPI result", result);
		if (result.status == 200) {
			yield put({
				type: BOARD_INFO_SUCCESS,
				data: result.data.data,
			});
		} else {
			yield put({
				type: BOARD_INFO_FAILURE,
				error: result.response,
			});
		}
	} catch (err) {
...
//음악 연관 상세 정보
function popualarKeywordRankAPI() {
	return axios.get(`/music/popularKeyword`);
}

function* popualarKeywordRank() {
	try {
		const result = yield call(popualarKeywordRankAPI);
		console.log("popualarKeywordRankAPI result", result);
		if (result.status == 200) {
			yield put({
				type: POPUALAR_SEARCH_KEYWORD_SUCCESS,
				data: result.data.data,
			});
		} else {
			yield put({
				type: POPUALAR_SEARCH_KEYWORD_FAILURE,
				error: result.response,
			});
		}
	} catch (err) {
...
function* watchBoardInfo() {
	yield takeLatest(BOARD_INFO_REQUEST, boardInfo);
}
...
function* watchPopualarKeywordRank() {
	yield takeLatest(POPUALAR_SEARCH_KEYWORD_REQUEST, popualarKeywordRank);
}
