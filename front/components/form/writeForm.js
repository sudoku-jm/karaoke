...
const WriteForm = ({ insertType }) => {
                ...
                case "MODIFY":
                    ...
                    if (musicInfoDone) {
                        ...
                        if (Musicdata.Links.length > 0) {
                            settingForm.link = Musicdata.Links.map(
                                (item) => item.src
                            ).join(", ");
                        }
    ...
    const handleRemoveTag = useCallback(
        (tag) => {
            const tags = insertForm.tags.replace(
                new RegExp(`#${tag}`, "g"),
                ""
            );
            setInsertForm((prev) => ({
                    ...
                    <div>
                        ...
                        {!Validation.isEmptyObject(resultData.newData) && (
                            <div>
                                ...
                                요청 가수 영문명 :{" "}
                                {resultData.newData.b_e_singer}
                                <br />
                                요청 가수 일본어명 :{" "}
                                {resultData.newData.b_j_singer}
                                <br />
                                금영번호 : {resultData.newData.b_keumyong}{" "}
                                <br />
                                태진번호 : {resultData.newData.b_taejin} <br />
                                카테고리 : {resultData.newData.b_category}{" "}
                                <br />
