...
router.post("/insertSinger", async (req, res, next) => {
  try {
    console.log("/admin/insertSinger=================================[START]");
    let form = {};
    ...
    console.log("/admin/insertSinger=================================[END]");
  } catch (error) {
    console.log(error);
    next(error);
  }
...
router.post("/insertCategory", async (req, res, next) => {
  try {
    ...
    console.log("/admin/insertCategory=================================[END]");
  } catch (error) {
    console.log(error);
    next(error);
  }
...
router.post("/insertMusic", async (req, res, next) => {
  try {
    console.log("/admin/insertMusic=================================[START]");
    ...
    if (req.body.boardId !== undefined && req.body.boardId !== "") {
      ...
      if (boardData !== null) {
        if (boardData.new == "Y") {
          ...
          if (boardData.b_keumyong == "" && boardData.b_taejin == "") {
            return res.status(202).json({
              msg: "음원 번호가 빈값입니다.",
            });
          }
          ...
          form = {
            title:
              boardData.b_title == "" ? musicData.title : boardData.b_title,
            keumyong:
              boardData.b_keumyong == ""
                ? musicData.keumyong
                : boardData.b_keumyong,
            taejin:
              boardData.b_taejin == "" ? musicData.taejin : boardData.b_taejin,
            CategoryId:
